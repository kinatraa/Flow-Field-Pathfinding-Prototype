...
public class FlowField
{
    ...
    public void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSize.x; x++)
        {
            ...
            {
                Vector3 worldPos = new Vector3(x * cellDiameter + cellRadius, y * cellDiameter + cellRadius, 0);
    ...
    public void CreateCostField()
    {
        ...
        int impassibleLayer = LayerMask.NameToLayer("Impassible");
        int roughLayer = LayerMask.NameToLayer("RoughTerrain");
        ...
        foreach (Cell curCell in grid)
        {
            ...
            {
                if (col.gameObject.layer == impassibleLayer)
                {
                    curCell.IncreaseCost(255);
                    continue;
                }
                else if (!hasIncreasedCost && col.gameObject.layer == roughLayer)
                {
    ...
    public void CreateIntegrationField(Cell _destinationCell)
    {
        ...
        while(cellsToCheck.Count > 0)
        {
            ...
            List<Cell> curNeighbors = GetNeighborCells(curCell.gridPos, GridDirection.CardinalDirections);
    ...
    public void CreateFlowField()
    {
        foreach(Cell curCell in grid)
        {
            List<Cell> curNeighbors = GetNeighborCells(curCell.gridPos, GridDirection.AllDirections);
            ...
            foreach(Cell curNeighbor in curNeighbors)
            {
                if(curNeighbor.bestCost < bestCost)
                {
                    ...
                    curCell.bestDirection = GridDirection.GetDirectionFromV2I(curNeighbor.gridPos - curCell.gridPos);
                }
            }
        }
    }
